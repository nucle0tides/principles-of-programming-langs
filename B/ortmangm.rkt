...
(define (eval expr env heap)
  (cond
    ...
    [ (equal? (car expr) 'wref)
      ;; then
      (wrefhandler (value (eval (cadr expr) env heap)) (value (eval (cadr (cdr expr)) env (cadr (eval (cadr expr) env heap))))
                   (cadr (eval (cadr (cdr expr)) env (cadr (eval (cadr expr) env heap))))) ]
...
(define (evalcond2 op semoperand1 operand2 env)
  (if (list? (value semoperand1)) 
      semoperand1
      (if (equal? operand2 'dummy) 
